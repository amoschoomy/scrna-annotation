...
X_train, X_test, y_train, y_test = train_test_split(
    X, y_onehot, test_size=0.33, random_state=42)
...
def build_model(hp):
    ...
    model.compile(optimizer=keras.optimizers.Adam(
        learning_rate=hp_learning_rate), loss='categorical_crossentropy', metrics=['accuracy'])
    return model
...
tuner = kt.BayesianOptimization(
    build_model, objective='val_accuracy', max_trials=5, seed=42)
